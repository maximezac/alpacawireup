...
# ---- ENV knobs (same as old publish_feed.py) ----
...
def last_price_and_ts(node: Dict[str, Any]) -> Tuple[float, Optional[str]]:
    """
    Pull best-available latest price & timestamp from a base node.
    ...
    """
    # Prefer intraday bars if present
...
# ---- TS logic (copied from publish_feed) ----
...
def compute_TS_intraday(ind: Dict[str, Any]) -> float:
    ...
    ts = 0.40 * ts_macd_slope + 0.35 * ts_rsi + 0.25 * ts_ema_trend
    return clamp_unit(ts)


# ---- top-level per-symbol computation ----
